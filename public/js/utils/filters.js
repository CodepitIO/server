...
app.filter("formatTime", function () {
  return function (seconds, trailingZero) {
    ...
    else if (m > 0) str = m + "min and " + s + "sec";
    else str = s + "sec";
    return str + suffix;
  };
});
...
app.filter("formatDuration", function () {
  return function (time, seconds) {
    ...
    if (seconds && (s > 0 || arr.length === 0)) arr.push(s + "sec");

    if (arr.length === 1) return arr[0];
    return _.join([_.join(_.initial(arr), ", "), _.last(arr)], " and ");
  };
});
...
app.filter(
  ...
  function (
    ...
    return function (date) {
      ...
      var link =
        '<a target="_blank" href="http://www.timeanddate.com/worldclock/fixedtime.html?day=' +
        date.getUTCDate() +
        "&month=" +
        (date.getUTCMonth() + 1) +
        "&year=" +
