  ...
  .factory(
    ...
    function ($resource, $state, Request, Notification, UserState) {
      var API = {
        ...
        edit: Request.send(
          "save",
          $resource("/api/v1/team/:id/edit", { id: "@id" })
        ),
        leave: Request.send(
          "save",
          $resource("/api/v1/team/:id/leave", { id: "@id" })
        ),
        invite: Request.send(
          "save",
          $resource("/api/v1/team/:id/invite", { id: "@id" })
        ),
        remove: Request.send(
          "save",
          $resource("/api/v1/team/:id/remove", { id: "@id" })
        ),
        accept: Request.send(
          "save",
          $resource("/api/v1/team/:id/accept", { id: "@id" })
        ),
        decline: Request.send(
          "save",
          $resource("/api/v1/team/:id/decline", { id: "@id" })
        ),
      ...
      return {
        create: function (name, callback) {
          ...
          if (name.length < 1 || name.length > 50) {
            return Notification.info(
              "The team name must have between 1 and 50 characters."
            );
          }
        ...
        invite: function (teamId, invitedEmail, callback) {
          API.invite({ id: teamId, invited: invitedEmail }).then(function (
            data
          ) {
