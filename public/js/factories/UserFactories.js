  ...
  .factory(
    ...
    function ($state, $resource, Request, Notification, UserState) {
      var API = {
        ...
        checkUsername: Request.send(
          "get",
          $resource("/api/v1/user/check/username/:username")
        ),
        checkEmail: Request.send(
          "get",
          $resource("/api/v1/user/check/email/:email")
        ),
        teams: Request.send("get", $resource("/api/v1/user/teams")),
        recover: Request.send("post", $resource("api/v1/user/recover")),
        sendPasswordRecoveryEmail: Request.send(
          "get",
          $resource("/api/v1/user/recover/:user")
        ),
        sendValidationEmail: Request.send(
          "get",
          $resource("/api/v1/user/validate")
        ),
      ...
      return {
          ...
          API.login(user).then(
            function (data) {
              if (data._id) {
                ...
                Notification.success("Welcome, " + data.local.name + "!");
              }
        ...
        sendPasswordRecoveryEmail: function (user, callback) {
          API.sendPasswordRecoveryEmail({ user: user }).then(function () {
            Notification.info("Password recovery e-mail sent.");
          });
        },

        sendValidationEmail: function (callback) {
          API.sendValidationEmail().then(function () {
            Notification.info("Verification e-mail sent.");
          });
        },

        validate: function (hash, callback) {
          API.validate({ hash: hash }).then(
            function (data) {
              UserState.set(data);
              $state.go("profile", { id: data._id });
              Notification.success("E-mail verified!");
            },
            function () {
              $state.go("home");
            }
        ...
        teams: function (callback) {
          ...
          API.teams().then(function (data) {
            return (
              callback &&
              callback(null, { member: data.member, invited: data.invited })
            );
          });
        },
