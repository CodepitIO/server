  ...
  async.parallel(
    {
      member: (next) => {
        Team.find({ members: userId }, "_id name description members")
          .populate({
            path: "members",
            select: "_id username local.name local.surname local.email",
          })
          .exec(next);
      },
      invited: (next) => {
    ...
    (err, results) => {
      ...
      results.member = _.map(results.member, (obj) => {
        return obj.toObject({ virtuals: true });
      });
      results.invited = _.map(results.invited, (obj) => {
        return obj.toObject({ virtuals: true });
      });
  ...
  async.parallel(
    {
      ...
      invited: (next) => {
        User.findOne(
          { "local.email": invitedEmail },
          "_id username local.name local.surname local.email",
          next
        );
      },
    },
