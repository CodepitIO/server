  ...
  async.waterfall(
    [
      ...
      (results, next) => {
        ...
        if (
          (results.problem &&
            !results.contest.problemInContest(submission.problem)) ||
          !results.contest.userInContest(userId)
        )
          return next(new Error());
        if (results.contest.date_start > new Date()) return next(new Error());
        if (!_.some(submission.language, results.contest.languages))
          return next(new Error());
...
exports.getRepContestSubmissions = (req, res) => {
  ...
  Contest.findById(contestId, (err, contest) => {
    ...
    if (!rep) return res.send(400).send();
    Submission.find({
      contest: contestId,
      rep: rep,
      date: { $gt: startFrom },
